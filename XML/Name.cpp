...
{
    return name._qname == _qname && name._namespaceURI == _namespaceURI && name._localName == _localName;
}

bool Name::equals(const XMLString& qname, const XMLString& namespaceURI, const XMLString& localName) const
{
    return _qname == qname && _namespaceURI == namespaceURI && _localName == localName;
}
...
XMLString Name::prefix(const XMLString& qname)
{
    XMLString::size_type pos = qname.find(':');
    if (pos != XMLString::npos)
        return XMLString(qname, 0, pos - 1);
    else
        return EMPTY_NAME;
}
